...
import tempfile
import time
from typing import Any, Iterable, Mapping, NoReturn, Optional, Sequence, Union
...
class CompletedProcess(subprocess.CompletedProcess):
    ...
    def cmdstr(self) -> str:
        ...
        if self.args is None:
            ...
            return shell_util.cmd_to_str(self.args)
...
class CalledProcessError(subprocess.CalledProcessError):
    ...
    def cmdstr(self):
        ...
        if self.cmd is None:
            ...
            return shell_util.cmd_to_str(self.cmd)
...
def run(
    ...
    **kwargs,
    ...
    if dryrun or print_cmd or log_output:
        ...
        if dryrun:
            log += "(dryrun) "
        log += "run: %s" % (shell_util.cmd_to_str(cmd),)
        if cwd:
            log += " in %s" % (cwd,)
